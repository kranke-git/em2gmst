...
species     = [ 'CO2red', 'CO2rem', 'ICO2', 'CH4', 'SF6', 'N2O', 'HFC' ] # List of species to analyze, should match the keys in the SPECIES dictionary
# Input ends
...
for scen_selection in scen_labels:
    ...
    for spec in species:
        ...
        if spec == 'CO2red' or spec == 'CO2rem' or spec == 'ICO2':
            cs_species = 'CO2'
        elif spec == 'HFC':
            cs_species = 'HFC-134a'
        else:
            cs_species = spec
        gaslabel       = f"{SPECIES[ cs_species ]['units']}{ cs_species}"
        df[ gaslabel ] = df["value"] * ( 1000 / SPECIES[ cs_species ]["GWP100"] ) * SPECIES[ cs_species ][ "emission_conv" ] # covert from credits to kgCH4, then to units used in the RF calculations (ppm for CO2, ppb for CH4)
        ...
        out_rf[f"{scen_selection}_{spec}"]         = cs_instance.outdf[f'RF_{cs_species}'].values
        out_conc[f"{scen_selection}_{spec}"]       = cs_instance.outdf[f'Catm_{cs_species}'].values - SPECIES[cs_species]["C_PI"] # Convert from concentration to emissions units (kg of that species emitted in that year)
        out_emissions[f"{scen_selection}_{spec}"]  = cs_instance.outdf[f'emissions_{cs_species}'].values 
        out_cumulative[f"{scen_selection}_{spec}"] = cs_instance.outdf[f'cumulativeEmissions_{cs_species}'].values
        # Plot the results for this scenario/species combination and save out the figure
        figScen = cs_instance.plotOutput( species = cs_species, title = f'Scenario:{scen_selection}, Species:{spec}', Catm_anomaly_flag = True )
        figScen.savefig( f"plots/{scen_selection}_{spec}_plot.png", dpi=300, bbox_inches="tight", pad_inches=0.05, facecolor="white" )

out_gmst.to_csv( "outdata/allscenarios_gmst.csv", index=False )
out_rf.to_csv( "outdata/allscenarios_rf.csv", index=False )
out_conc.to_csv( "outdata/allscenarios_conc.csv", index=False )
out_emissions.to_csv( "outdata/allscenarios_emissions.csv", index=False )
out_cumulative.to_csv( "outdata/allscenarios_cumulative.csv", index=False )
...
colors_noICO2 = colors[ [i for i, s in enumerate(species) if s != 'ICO2'] ] # Exclude the color for ICO2
...
fig = plot_gmst_stacked( out_gmst, [s for s in species if s != 'ICO2'], scen_labels, colors = colors_noICO2 )
...
fig = plot_gmst_totals(out_gmst, [s for s in species if s != 'ICO2'], scen_labels, title = 'Total GMST without impermanent CO2')
